...
export const GET_COMMENTS_SCHEMA = {
  block_id: z.string().describe("The ID of the block or page to get comments from"),
  start_cursor: z.string().describe("The cursor to start from for pagination").optional(),
  page_size: z.number().describe("Number of comments to return per page").optional(),
};
...
export const ADD_PAGE_COMMENT_SCHEMA = {
  ...
  }).describe("The parent page of the comment."),
  rich_text: z
...
export const ADD_DISCUSSION_COMMENT_SCHEMA = {
  discussion_id: z.string().describe("The ID of the discussion to add the comment to"),
  rich_text: z
    .array(RICH_TEXT_ITEM_REQUEST_SCHEMA)
    .describe("Rich text content for the comment"),
};
